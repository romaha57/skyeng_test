...
def send_email_after_check(user_id: int, record_id: int, message_text: str) -> None:
    """Отложенная таска для отправки письма на почту и изменения статуса файла на is_check=True"""

    user = User.objects.get(pk=user_id)
    upload_file = UploadFile.objects.get(pk=record_id)

    # отмечаем в бд правильность файла
    if 'успешно' in message_text:
        upload_file.is_right = True
    else:
        upload_file.is_right = False

    upload_file.is_check = True
    upload_file.save()
